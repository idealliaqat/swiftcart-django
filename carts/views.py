    ...
    if current_user.is_authenticated:
        ...
        if is_cart_item_exist:
            ...
            for item in cart_item:
                ...
                id.append(item.id)
            if product_variation in ex_var_list:  # check selected or added product variation is must be present in assigned or offered variations
...
def checkout(request, total=0, grand_total=0, quantity=0, tax=0, cart_items=None):
    try:
        if request.user.is_authenticated:
            cart_items = CartItem.objects.filter(user=request.user, is_active=True)
        else:
